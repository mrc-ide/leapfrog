...
* Add new input data - modify [`leapfrogr/inst/cpp_generation/model_input.csv`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrogr/inst/cpp_generation/model_input.csv) and run [`scripts/generate`](https://github.com/mrc-ide/leapfrog/blob/main/scripts/generate)
* Add new model output - modify [`leapfrogr/inst/cpp_generation/model_output.csv`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrogr/inst/cpp_generation/model_output.csv) and run [`scripts/generate`](https://github.com/mrc-ide/leapfrog/blob/main/scripts/generate)

### Input data

Input data is generated from csv file in [`leapfrogr/inst/include/model_input.hpp`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrogr/inst/include/model_input.hpp), the columns in this file are
...
After making changes to the [`model_input.csv`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrogr/inst/cpp_generation/model_input.csv), run the generate script [`scripts/generate`](https://github.com/mrc-ide/leapfrog/blob/main/scripts/generate). This update file [`leapfrogr/inst/include/model_input.hpp`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrogr/inst/include/model_input.hpp). Note that this file should never be changed manually as the generate script will completely rewrite it.

After regenerating the code, rebuild the project and you are ready to use the new input data in C++.

### Model outputs

Model outputs are generated in a similar way to inputs. To modify an existing or add new output update the file [`leapfrogr/inst/cpp_generation/model_output.csv`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrogr/inst/cpp_generation/model_. The columns in this file are:
...
The intermediate data is not handled by code generation as it only requires changes in one place. You can make modifications by adding new data into the struct in [`leapfrogr/inst/include/types.hpp`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrogr/inst/include/types.hpp), making sure to set its default value in the `reset` function on the struct.
