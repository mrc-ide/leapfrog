...
Each model variant can have state space, input parameters, output state and intermediate data associated with it. Within the model code for each variant it can be useful to unpack the structs into a shorthand to save typing. We do this in a consistent way based on the following rules:

1. Prefix based on what is being unpacked, `parameters`, `state_curr`, `state_next` or `intermediate`
   1. `p_` for `pars`
   1. `c_` for `state_curr`
   1. `n_` for `state_next`
   1. `i_` for `intermediate`
1. 2nd prefix based on what model variant the data belongs to. This comes from the `namespace` value for the model variant from the config.
   1. `dp` for the demographic projection
   1. `ha` for the HIV adult model
   1. `hc`for the HIV child model
   1. `sp` for the Spectrum model
...
For the `StateSpace` struct on the config we can unpack this into the struct for each model variant so the state space can be used directly.
...
|                    | children.age_specific_fertility_rate     | Rate of live births in a year and the total female population of childbearing age (needs to be a separate input from the demography parameters as this always need to be fully stratified)                                                                                                                                               |
...
|                    | children.ctx_val_is_percent              | Number of children receiving cotrimoxazole                                                                                                                            |
...
|                    | children.hc_art_is_age_specific          | Boolean vector indicating whether ART is input as age-specific values or for all children 0-14y                                                                       |
...
|                    | children.hc_art_ltfu                     | Treatment interruption among children on treatment 0-14y                                                                                                              |
| fert_mult_by_age   | children.fert_mult_by_age                | Fertility ratio for WLHIV by age                                                                                                                                      |
| fert_mult_offart   | children.fert_mult_off_art               | Fertility ratio for WLHIV not on treatment by CD4                                                                                                                     |
| fert_mult_onart    | children.fert_mult_on_art                | Fertility ratio for WLHIV on treatment by age                                                                                                                         |
| tfr                | children.total_fertility_rate            | Total fertility rate                                                                                                                                                  |
| laf                | children.local_adj_factor                | Local fertility adjustment to match programmatic data                                                                                                                 |
| pmtct              | children.PMTCT                           | Number or percent of women receiving ARVs for PMTCT  (order: option A, Option B, SDNVP, Dual ARV, Option B+: before pregnancy, Option B+: >4 weeks, Option B+:<4 weeks)                                                                                                                 |
| mtct               | children.vertical_transmission_rate      | Vertical transmission rate for pregnant and breastfeeding women not receiving ARV by CD4 category                                                                     |
...
| mat_hiv_births     | children.mat_hiv_births                  | Number of births to WLHIV (needed to bypass the adult model).                                                                                                         |
| mat_prev_input     | children.mat_prev_input                  | Boolean vector indicating if input mat_hiv_births should be used for vertical transmission rather than results from adult model (needed to bypass the adult model).    |
| prop_lt200         | children.prop_lt200                      | Proportion of WLHIV with CD4 less than 200 by year (needed to bypass the adult model).                                                                                |
| prop_gte350        | children.prop_gte350                     | Proportion of WLHIV with CD4 greater than 350 by year  (needed to bypass the adult model).                                                                            |
|                    | children.adult_female_infections         | Age-specific number of infections among women 15-49 by 5 year age group (needed to bypass the adult model).                                                         |
|                    | children.adult_female_hivnpop            | Age-specific number of HIV negative women 15-49 by 5 year age group  (needed to bypass the adult model).                                                         |
|                    | children.infant_pop                      | Sex and age specific infant population  (needed to bypass the adult model).                                                                                           |
|                    | children.total_births                    | Number of total births  (needed to bypass the adult model).                                                                                                           |
|                    | children.total_births                    | Number of total births  (needed to bypass the adult model).                                           |                    | children.abortion                        | Number of pregnancy terminations among women living with HIV.                                                                                                         |
|                    | children.patients_reallocated            | Number of pregnant women reallocated to receive ART from inside or outside of the geographical area represented in the Spectrum file.                               |

## Outputs

| Leaptfrog         | Leapfrog                    | Details                                                                  |
|------------------|----------------------------|--------------------------------------------------------------------------|
| totpop1          | p_totpop                | Projected total population                                               |
| hivpop1          | p_hivpop                  | Projected HIV population                                                 |
| infections       | p_infections               | Projected number of new HIV infections by sex and age                    |
| hivstrat_adult   | h_hivpop                | Projected PLHIV not on ART by age, sex and CD4                           |
| artstrat_adult   | h_artpop                | Projected PLHIV on ART by age, sex, CD4 and treatment stage              |
| births           | births                     | Projected number of births                                               |
| natdeaths        | p_deaths_background_totpop | Projected number of background deaths                                       |
| natdeaths_hivpop | p_deaths_background_hivpop   | Projected number of background deaths within HIV population                 |
| hivdeaths        | p_hiv_deaths               | Projected HIV-related deaths by sex and age                              |
| netmig_ag        | p_net_migration_hivpop     | Number of HIV+ Migrants                                                  |
...
| hivstrat_paeds   | hc1_hivpop                | Projected PLHIV 0-4y not on ART by age, sex, transmission type, and CD4  |
| hivstrat_paeds   | hc2_hivpop                | Projected PLHIV 5-14y not on ART by age, sex, transmission type, and CD4 |
| artstrat_paeds   | hc1_artpop                | Projected PLHIV 0-4y on ART by age, sex, treatment stage and CD4         |
| artstrat_paeds   | hc2_artpop                | Projected PLHIV 5-14y on ART by age, sex, treatment stage and CD4        |
...
|                  | hc_art_need_init           | Children who are eligible for ART by CD4, transmission, age, sex, and year                 |
|                  | hc_art_init                | Number of new ART initiates by five year age group.                                        |
|                  | ctx_need                   | Number of children in need of cotrim                                                       |
|                  | infecton_by_type           | Number of paediatric HIV infections by type of infection (peri vs bf) and age at infection.|
|                  | ctx_need                   | Number of children in need of cotrim                                                       |
|                  | mtct_by_source_tr          | Transmission rate by maternal treatment history (stacked bar output)                       |
|                  | mtct_by_source_women       | Number of pregnant WLHIV by maternal treatment history (stacked bar output)                |
|                  | mtct_by_source_hc_infections | Number of paediatric infections by maternal treatment history (stacked bar output)       |
|                  | hiv_births_by_mat_age      | Number of births to HIV positive women by maternal age                                     |
...
| hivpop_ha(ha, s)          | hiv_population_coarse_ages      |                                                                           |
...
| hivqx_ha                  |                                 |                                                                           |
| hivdeaths_a               |                                 |                                                                           |
| age15_hivpop              | age15_hivpop                    | 15 year olds aging into the adult population                              |
