 ...
 hiv_births.attr("dim") = NumericVector::create( proj_years);
  ...
  if (hAG == hAG_FULL) {
    leapfrog_sim<double, NG, pAG, pIDX_FERT, pAG_FERT,
		 pIDX_HIVADULT, hAG_FULL, hDS, hDS_adol, hTM, hTS, hPS, hBF>
      (REAL(demp["basepop"]),
       REAL(demp["Sx"]),
       ...
       *INTEGER(projp["scale_cd4_mort"]),
       ...
       REAL(projp["mtct"]),
       REAL(projp["pmtct"]),
       REAL(projp["bf_duration"]),
       REAL(projp["pmtct_dropout"]),
       REAL(projp["mat_hiv_births"]),
       REAL(projp["prop_gte350"]),
       REAL(projp["prop_lt200"]),
       proj_years,
       hiv_steps_per_year,
    ...
    leapfrog_sim<double, NG, pAG, pIDX_FERT, pAG_FERT,
		 pIDX_HIVADULT, hAG_COARSE, hDS, hDS_adol, hTM, hTS, hPS, hBF>
      (REAL(demp["basepop"]),
       REAL(demp["Sx"]),
       REAL(demp["netmigr_adj"]),
       REAL(demp["asfr"]),
