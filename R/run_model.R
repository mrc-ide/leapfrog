...
#' Run run_wlhiv_births c++ function without running adult model
#'
#' @param data Input data
#' @param parameters Projection parameters
#' @param sim_years Simulation years to run model for default 1970:2030
...
#' @return List of model outputs
#' @export
run_wlhiv_births <- function(data, parameters, sim_years,
                             hts_per_year, data_from_adult_model,
                             output_steps = NULL) {
  if (is.null(sim_years)) {
    sim_years <- 1970:2030
  }
  ...
  run_wlhiv_births_r(data, sim_years, hts_per_year, output_steps, data_from_adult_model)
}
