  ...
  for (int g = 0; g < num_genders; g++) {
    ...
    for (int a = 1; a < age_groups_pop; a++) {
      state_next.natural_deaths(a, g) =
          state_cur.total_population(a - 1, g) * (1.0 - pars.survival(a, g));
      state_next.total_population(a, g) = state_cur.total_population(a - 1, g) -
                                          state_next.natural_deaths(a, g);
    }

    // open age group
    real_type natural_deaths_open_age =
        state_cur.total_population(age_groups_pop - 1, g) *
