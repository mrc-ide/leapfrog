      ...
      for (int a = 0; a < hc_ss.hc2_agestart; ++a) {
        for (int cat = 0; cat < hc_ss.hcTT; ++cat) {
          intermediate.children.hc_posthivmort(hd, cat, a, s) += state_next.children.hc1_hiv_pop(hd, cat, a, s) -
                                                                 (1.0 - cpars.ctx_effect * cpars.ctx_val(time_step)) *
                                                                 state_curr.children.hc1_hiv_pop(hd, cat, a, s) *
                                                                 cpars.hc1_cd4_mort(hd, cat, a);
        }
      }
    }
  }
      ...
      for (int a = 0; a < pars.base.options.p_idx_fertility_first; ++a) {
        for (int cat = 0; cat < hc_ss.hcTT; ++cat) {
          if (a < hc_ss.hc2_agestart) {
            intermediate.children.hc_grad(hd, cat, a, s) -=
                (1 - cpars.ctx_effect * cpars.ctx_val(time_step)) * state_next.children.hc1_hiv_pop(hd, cat, a, s) *
                cpars.hc1_cd4_mort(hd, cat, a);
          } else {
            intermediate.children.hc_grad(hd, cat, a, s) -= (1 - cpars.ctx_effect * cpars.ctx_val(time_step)) *
                                                            state_next.children.hc2_hiv_pop(hd, cat,
                                                                                            a - hc_ss.hc2_agestart, s) *
                                                            cpars.hc2_cd4_mort(hd, cat, a - hc_ss.hc2_agestart);
