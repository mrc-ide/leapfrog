...
void project_year(int t,
  ...
  if constexpr (ModelVariant::run_hiv_simulation) {
      run_hiv_pop_demographic_projection<ModelVariant>(t, pars, state_curr, state_next,
                                                       intermediate);
      run_hiv_model_simulation<ModelVariant>(t, pars, state_curr, state_next, intermediate);
    }
    if constexpr (ModelVariant::run_child_model) {
      run_child_model_simulation<ModelVariant>(t, pars, state_curr, state_next, intermediate);
    }
    const auto& p_op = pars.options;

    if (p_op.proj_period_int == internal::PROJPERIOD_CALENDAR) {
      run_end_year_migration<ModelVariant>(t, pars, state_curr, state_next, intermediate);
      if constexpr (ModelVariant::run_hiv_simulation) {
        run_hiv_pop_end_year_migration<ModelVariant>(t, pars, state_curr, state_next, intermediate);
      }
    }
  }
} // namespace internal
