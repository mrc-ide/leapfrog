...
struct ChildModelSimulation<Config> {
  ...
  void calc_hiv_negative_pop() {
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = 0; a < hAG; ++a) {
        i_hc.p_hiv_neg_pop(a, s) = p_dp.base_pop(a, s) - n_ha.p_hiv_pop(a, s);
      }// end a
    }// end s
  };
        ...
        for (int hd = 0; hd < hc1DS; ++hd) {
          if (s == 0) {
            n_hc.hc1_hiv_pop(hd, 0, 0, 0) += perinatal_transmission_births *
                                             p_dp.births_sex_prop(0, t) * p_hc.hc1_cd4_dist(hd);
          } else {
            n_hc.hc1_hiv_pop(hd, 0, 0, s) += perinatal_transmission_births *
                                             (1 - p_dp.births_sex_prop(0, t)) * p_hc.hc1_cd4_dist(hd);
          }
        } // end hc1DS
  ...
  void need_for_cotrim() {
        ...
        for (int cat = 0; cat < hcTT; ++cat) {
          for (int hd = 0; hd < hc1DS; ++hd) {
            if (a < hc2_agestart) {
              ...
              i_hc.hc_ctx_need(hd, cat, a, s) += n_hc.hc2_hiv_pop(hd, cat, a - hc2_agestart, s);
            }
          } // end hc1DS
        } // end a
      } // end hcTT
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = 1; a < opts.p_idx_fertility_first; ++a) {
        for (int cat = 0; cat < hcTT; ++cat) {
          for (int hd = 0; hd < hc1DS; ++hd) {
            if (a == 1) {
              n_hc.ctx_need += n_hc.hc1_hiv_pop(hd, cat, a, s) * 0.5;
            } else if (a < hc2_agestart) {
  ...
  void cd4_mortality() {
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = hc2_agestart; a < opts.p_idx_fertility_first; ++a) {
        for (int cat = 0; cat < hcTT; ++cat) {
          for (int hd = 0; hd < hc2DS; ++hd) {
            auto hiv_deaths_strat = n_hc.ctx_mean * n_hc.hc2_hiv_pop(hd, cat, a - hc2_agestart, s) *
  ...
  void on_art_mortality(int t_art_idx) {
    const auto& p_hc = pars.hc;
    auto& n_hc = state_next.hc;
    auto& i_hc = intermediate.hc;
        ...
        for (int hd = 0; hd < hc1DS; ++hd) {
          for (int a = 0; a < opts.p_idx_fertility_first; ++a) {
            if (a < hc2_agestart) {
              ...
              i_hc.hc_art_deaths(p_hc.hc_age_coarse(a)) += n_hc.hc2_art_aids_deaths(dur, hd, a - hc2_agestart, s);
            }
          } // end a
        } // end hc1DS
      } // end NS
