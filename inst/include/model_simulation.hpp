...
void distribute_rate_over_sexes(
  ...
  real_type denominator = intermediate.base.hiv_neg_aggregate(MALE) +
                          incidence.relative_risk_sex(time_step) * intermediate.base.hiv_neg_aggregate(FEMALE);
  real_type total_neg = intermediate.base.hiv_neg_aggregate(MALE) + intermediate.base.hiv_neg_aggregate(FEMALE);
  intermediate.base.rate_sex(MALE) = incidence.total_rate(time_step) * (total_neg) / denominator;
      ...
      for (int hm = 0; hm < ss.hDS; ++hm) {
        ...
        if (pars.base.options.scale_cd4_mortality &&
          ...
          for (int hu = 0; hu < ss.hTS; ++hu) {
            intermediate.base.artpop_hahm += state_next.base.h_art_adult(hu, hm, ha, g);
          }
          intermediate.base.cd4mx_scale = state_next.base.h_hiv_adult(hm, ha, g) /
                                          (state_next.base.h_hiv_adult(hm, ha, g) + intermediate.base.artpop_hahm);
        }

        intermediate.base.deaths =
            intermediate.base.cd4mx_scale * natural_history.cd4_mortality(hm, ha, g) *
            state_next.base.h_hiv_adult(hm, ha, g);
        intermediate.base.p_hiv_deaths_age_sex(ha, g) += dt * intermediate.base.deaths;
        state_next.base.h_hiv_deaths_no_art(hm, ha, g) += dt * intermediate.base.deaths;
  ...
  for (int g = 0; g < ss.NS; g++) {
    ...
    for (int ha = 0; ha < ss.hAG; ++ha) {
      ...
      for (int i = 0; i < ss.hAG_span[ha]; i++, a++) {
        intermediate.base.p_infections_a = intermediate.base.p_infections_ts(a, g);
        intermediate.base.p_infections_ha += intermediate.base.p_infections_a;
        state_next.base.p_infections(a, g) += pars.base.options.dt * intermediate.base.p_infections_a;
        state_next.base.p_hiv_pop(a, g) += pars.base.options.dt * intermediate.base.p_infections_a;
      }

      // add p_infections to grad hivpop
      for (int hm = 0; hm < ss.hDS; ++hm) {
        intermediate.base.grad(hm, ha, g) +=
            intermediate.base.p_infections_ha * natural_history.cd4_initial_distribution(hm, ha, g);
      }
    }
  }
}
    ...
    for (int ha = 0; ha < ss.hAG; ha++) {
      for (int hm = intermediate.base.everARTelig_idx; hm < ss.hDS; hm++) {
        for (int hu = 0; hu < ss.hTS; hu++) {
          ...
          intermediate.base.p_hiv_deaths_age_sex(ha, g) += pars.base.options.dt * intermediate.base.deaths_art;
          state_next.base.h_hiv_deaths_art(hu, hm, ha, g) += pars.base.options.dt * intermediate.base.deaths_art;
...
void run_h_art_initiation(int hiv_step,
  ...
  for (int g = 0; g < ss.NS; ++g) {
    ...
    for (int ha = hIDX_15PLUS; ha < ss.hAG; ++ha) {
      for (int hm = intermediate.base.everARTelig_idx; hm < ss.hDS; ++hm) {
        if (hm >= intermediate.base.anyelig_idx) {
          ...
          intermediate.base.Xartelig_15plus += intermediate.base.artelig_hahm(hm, ha - hIDX_15PLUS);
          intermediate.base.expect_mort_artelig15plus +=
              natural_history.cd4_mortality(hm, ha, g) * intermediate.base.artelig_hahm(hm, ha - hIDX_15PLUS);
        }
        for (int hu = 0; hu < ss.hTS; ++hu)
          intermediate.base.Xart_15plus +=
              state_next.base.h_art_adult(hu, hm, ha, g) + dt * intermediate.base.gradART(hu, hm, ha, g);
      }
    }
    ...
    if (dt * (hiv_step + 1) < 0.5) {
      if (!art.adults_on_art_is_percent(g, time_step - 2) && !art.adults_on_art_is_percent(g, time_step - 1)) {
        ...
        intermediate.base.artcov_hts =
            ...
            (dt * (hiv_step + 1) + 0.5) * art.adults_on_art(g, time_step - 1);
        intermediate.base.artnum_hts =
            intermediate.base.artcov_hts * (intermediate.base.Xart_15plus + intermediate.base.Xartelig_15plus);
      } else if (!art.adults_on_art_is_percent(g, time_step - 2) && art.adults_on_art_is_percent(g, time_step - 1)) {
        // Transition from number to percentage
        intermediate.base.curr_coverage =
            intermediate.base.Xart_15plus / (intermediate.base.Xart_15plus + intermediate.base.Xartelig_15plus);
        intermediate.base.artcov_hts = intermediate.base.curr_coverage +
                                       (art.adults_on_art(g, time_step - 1) - intermediate.base.curr_coverage) * dt /
                                       (0.5 - dt * hiv_step);
        intermediate.base.artnum_hts =
            intermediate.base.artcov_hts * (intermediate.base.Xart_15plus + intermediate.base.Xartelig_15plus);
      }
      ...
      if (!art.adults_on_art_is_percent(g, time_step - 1) && !art.adults_on_art_is_percent(g, time_step)) {
        ...
        intermediate.base.curr_coverage =
            intermediate.base.Xart_15plus / (intermediate.base.Xart_15plus + intermediate.base.Xartelig_15plus);
        intermediate.base.artcov_hts = intermediate.base.curr_coverage +
                                       (art.adults_on_art(g, time_step) - intermediate.base.curr_coverage) * dt /
                                       (1.5 - dt * hiv_step);
        intermediate.base.artnum_hts =
            intermediate.base.artcov_hts * (intermediate.base.Xart_15plus + intermediate.base.Xartelig_15plus);
      }
    ...
    intermediate.base.artinit_hts =
        intermediate.base.artnum_hts > intermediate.base.Xart_15plus ? intermediate.base.artnum_hts -
                                                                       intermediate.base.Xart_15plus : 0.0;
      ...
      for (int hm = intermediate.base.anyelig_idx; hm < ss.hDS; ++hm) {
        if (intermediate.base.Xartelig_15plus > 0.0) {
          intermediate.base.artinit_hahm =
              intermediate.base.artinit_hts * intermediate.base.artelig_hahm(hm, ha - hIDX_15PLUS) *
              ((1.0 - pars.base.options.initiation_mortality_weight) /
               ...
               intermediate.base.expect_mort_artelig15plus);
          if (intermediate.base.artinit_hahm > intermediate.base.artelig_hahm(hm, ha - hIDX_15PLUS)) {
            intermediate.base.artinit_hahm = intermediate.base.artelig_hahm(hm, ha - hIDX_15PLUS);
          }
          if (intermediate.base.artinit_hahm >
              state_next.base.h_hiv_adult(hm, ha, g) + dt * intermediate.base.grad(hm, ha, g)) {
            intermediate.base.artinit_hahm =
                state_next.base.h_hiv_adult(hm, ha, g) + dt * intermediate.base.grad(hm, ha, g);
          }
          intermediate.base.grad(hm, ha, g) -= intermediate.base.artinit_hahm / dt;
          intermediate.base.gradART(ART0MOS, hm, ha, g) += intermediate.base.artinit_hahm / dt;
          state_next.base.h_art_initiation(hm, ha, g) += intermediate.base.artinit_hahm;
        }
      }
    }
  }
...
void run_update_art_stratification(int hiv_step,
  ...
  for (int g = 0; g < ss.NS; ++g) {
    for (int ha = 0; ha < ss.hAG; ++ha) {
      for (int hm = intermediate.base.everARTelig_idx; hm < ss.hDS; ++hm) {
        for (int hu = 0; hu < ss.hTS; ++hu) {
          state_next.base.h_art_adult(hu, hm, ha, g) += pars.base.options.dt * intermediate.base.gradART(hu, hm, ha, g);
        }
      }
    }
  }
...
void run_update_hiv_stratification(int hiv_step,
  ...
  for (int g = 0; g < ss.NS; ++g) {
    for (int ha = 0; ha < ss.hAG; ++ha) {
      for (int hm = 0; hm < ss.hDS; ++hm) {
        state_next.base.h_hiv_adult(hm, ha, g) += pars.base.options.dt * intermediate.base.grad(hm, ha, g);
      }
    }
  }
}
  ...
  for (int g = 0; g < ss.NS; ++g) {
    ...
    for (int ha = 0; ha < ss.hAG; ++ha) {
      if (intermediate.base.hivpop_ha(ha) > 0) {
        intermediate.base.hivqx_ha = intermediate.base.p_hiv_deaths_age_sex(ha, g) / intermediate.base.hivpop_ha(ha);
        for (int i = 0; i < ss.hAG_span[ha]; ++i, ++a) {
          intermediate.base.hivdeaths_a = state_next.base.p_hiv_pop(a, g) * intermediate.base.hivqx_ha;
