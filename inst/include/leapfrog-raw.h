...
template <typename Type, int NG, int pAG, int pIDX_FERT, int pAG_FERT,
  ...
  void leapfrog_sim(const Type *p_basepop,
                    ...
                    //input adults for uncertainty analysis
...
//TODO ABORTION
   ...
   proplte350 = 1 - prop_gte350(t);
   propgte350 = prop_gte350(t);
   proplt200 = prop_lt200(t);
   prop200to350 = 1 - prop_gte350(t) - prop_lt200(t);
   total = 2;
 ...
 //    IncRate = 0;
  // }
   ...
   v3 = IncRate * (9/12) * (births_sum - need) * 0;//*0.181;
    ...
    for(int g = 0; g < NG; g++){
      for(int cat = 0; cat < hTM; cat++){
        for(int af = paed_art_elig_age(t); af < pIDX_HIVADULT; af++){
          for(int hm = 0; hm < hDS; hm++){
              need_art_paed(hm, cat, af, g) += (hm > (paed_art_elig_cd4(af, t))) ? hivstrat_paeds(hm, cat, af, g, t) : 0.0;
          }
        }
      }
    }
...
//  for(int hm = 0; hm < hDS; hm++){
         //   for(int cat = 0; cat < 1; cat++){
             ...
             hivstrat_paeds(0, 0, af, g, t) +=  infections(af, g, t) ;
          //  }
         // }
        }
     }
