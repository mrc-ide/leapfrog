...
This time output is managed by the state saver see [`leapfrog/inst/include/state_saver.hpp`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrog/inst/include/state_saver.hpp)
...
* Add new input data - modify [`inst/cpp_generation/model_input.csv`](https://github.com/mrc-ide/leapfrog/blob/main/inst/cpp_generation/model_input.csv) and run [`scripts/generate`](https://github.com/mrc-ide/leapfrog/blob/main/scripts/generate)
* Add new model output - modify [`inst/cpp_generation/model_output.csv`](https://github.com/mrc-ide/leapfrog/blob/main/inst/cpp_generation/model_output.csv) and run [`scripts/generate`](https://github.com/mrc-ide/leapfrog/blob/main/scripts/generate)

### Input data

Input data is generated from csv file in [`leapfrog/inst/include/model_input.hpp`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrog/inst/include/model_input.hpp), the columns in this file are
...
Note that the dims can be any of the state space sizes or `proj_years` for the number of years in the projection. See the [README](https://mrc-ide.github.io/leapfrog/#state-space) for valid state space sizes.

After making changes to the [`model_input.csv`](https://github.com/mrc-ide/leapfrog/blob/main/inst/cpp_generation/model_input.csv), run the generate script [`scripts/generate`](https://github.com/mrc-ide/leapfrog/blob/main/scripts/generate). This update file [`leapfrog/inst/include/model_input.hpp`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrog/inst/include/model_input.hpp). Note that this file should never be changed manually as the generate script will completely rewrite it.

After regenerating the code, rebuild the project and you are ready to use the new input data in C++.

### Model outputs

Model outputs are generated in a similar way to inputs. To modify an existing or add new output update the file [`inst/cpp_generation/model_output.csv`](https://github.com/mrc-ide/leapfrog/blob/main/inst/cpp_generation/model_. The columns in this file are:
...
The intermediate data is not handled by code generation as it only requires changes in one place. You can make modifications by adding new data into the struct in [`leapfrog/inst/include/types.hpp`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrog/inst/include/types.hpp), making sure to set its default value in the `reset` function on the struct.
...
1. Add any new input or output data in [`leapfrog/inst/include/model_input.csv`](https://github.com/mrc-ide/leapfrog/blob/main/inst/cpp_generation/model_input.csv) or [`leapfrog/inst/include/model_output.csv`](https://github.com/mrc-ide/leapfrog/blob/main/inst/cpp_generation/model_output.csv)
...
1. Add your new variant in [`leapfrog/inst/include/model_variants.hpp`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrog/inst/include/model_variants.hpp)
1. Add a struct for the new state space in [`leapfrog/inst/include/state_space.hpp`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrog/inst/include/state_space.hpp)
1. If your model has new input data, add parameters for this variant in [`leapfrog/inst/include/parameters.hpp`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrog/inst/include/parameters.hpp). Parameter types will be generated, you need to add those types to the appropriate template specialisation
1. If your model has new output data, add a new specialisation in [`leapfrog/inst/include/state_saver.hpp`](https://github.com/mrc-ide/leapfrog/blob/main/leapfrog/inst/include/state_saver.hpp)

You now need to update the R wrapper to be able to call this new variant:

1. In [`R/run_model.R`](https://github.com/mrc-ide/leapfrog/blob/main/R/run_model.R) add a new flag to the `run_model` interface for using your new variant
1. Update the conditionals to build the `model_variant` string based upon your new flag
1. Add mapping from the string to the model variant struct in Rcpp wrapper code in [`src/leapfrog.cpp`](https://github.com/mrc-ide/leapfrog/blob/main/src/leapfrog.cpp). Note use a `constexpr` for the state space to ensure it is evaluated at compile time.
