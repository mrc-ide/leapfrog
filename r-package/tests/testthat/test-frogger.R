test_that("initial state set up works as expected", {
  ...
  expect_setequal(
    ...
    c(
      ...
      "h_art_initiation", "p_hiv_deaths", "hiv_net_migration"
    )
  )
  ...
  expect_equal(out$hiv_net_migration, array(rep(0, 81 * 2), dim = c(81, 2, 1)))
})
...
test_that("model for 1 time step has looped", {
  ...
  expect_true(all(out$hiv_net_migration == 0))
})
