...
struct HivDemographicProjection<Config> {
  ...
  void run_hiv_pop_end_year_migration() {
    ...
    for (int g = 0; g < NS; ++g) {
      for (int a = 0; a < pAG; ++a) {
        n_ha.hiv_net_migration(a, g) = n_ha.p_hiv_pop(a, g) * i_dp.migration_rate(a, g);
        n_ha.p_hiv_pop(a, g) += n_ha.hiv_net_migration(a, g);
      }
    }
    ...
    for (int g = 0; g < NS; ++g) {
      ...
      for (int ha = 0; ha < hAG; ++ha) {
        ...
        for (int i = 0; i < hAG_span[ha]; ++i, ++a) {
          ...
          migration_num_ha += n_ha.hiv_net_migration(a, g);
        }
  ...
  void run_hc_hiv_pop_end_year_migration() {
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = 0; a < hcAG_end; ++a) {
        ...
        hc_migration_num = n_ha.hiv_net_migration(a, s);
    ...
    for (int g = 0; g < NS; ++g) {
      ...
      for (int ha = 0; ha < hAG; ++ha) {
        ...
        for (int i = 0; i < hAG_span[ha]; ++i, ++a) {
          ...
          if (opts.proj_period_int == PROJPERIOD_MIDYEAR) {
            deaths_migrate += n_ha.hiv_net_migration(a, g);
          }
        }
  ...
  void run_hc_hiv_and_art_stratified_deaths_and_migration() {
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = 0; a < hcAG_end; ++a) {
        ...
        if (opts.proj_period_int == PROJPERIOD_MIDYEAR) {
          deaths_migrate += n_ha.hiv_net_migration(a, s);
        }
