test_that("Child model can be run for all years", {
  ...
  expect_setequal(
    ...
    c(
      ...
      "hc1_art_aids_deaths_strat", "hc2_art_aids_deaths_strat",
...
test_that("Model outputs are consistent", {
  ...
  ##BF 6-12
  strat <- apply(out$infection_by_type[3,,,], c(2,3), sum)
  pop <- apply(out$hc_infections_coarse[,3,,,], c(3,4), sum)
  expect_equal(strat, pop)

  ##BF 12-24
  strat <- out$infection_by_type[4,2,,]
  pop <- apply(out$hc_infections_coarse[,4,2,,], c(2,3), sum)
  expect_equal(strat, pop)
